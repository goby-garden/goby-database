...
export default class Project {
    ...
    refresh_junction_cache() {
        ...
        this.junction_cache = junction_list_parsed;
    }
    ...
    action_create_class(name, meta) {
        ...
        const class_meta = {
            ...
            style: {
                color: '#b5ffd5',
                display: (meta === null || meta === void 0 ? void 0 : meta.display) || true,
                position: (meta === null || meta === void 0 ? void 0 : meta.position) || [0, 0]
            }
        };
    ...
    action_add_data_property(class_id, name, conditions, datatype, style) {
        ...
        let sql_datatype = '';
        if (conditions.max > 1 || text_datatypes.includes(datatype)) {
            //multiple for data means a stringified array no matter what it is
            sql_datatype = 'TEXT';
        }
        else if (real_datatypes.includes(datatype)) {
            sql_datatype = 'REAL';
        }
    ...
    action_edit_class_schema(edits) {
                ...
                if (change.class_id == undefined) {
                    ...
                    if (junction_list)
                        junction_list.map(junction => {
                            ...
                            if (matching)
                                matching.class_id = change.class_id;
                        });
                }
        ...
        if (junction_list !== undefined)
            this.action_update_relations(junction_list);
    ...
    action_update_relations(junction_list) {
        ...
        function remove_target(prop, target) {
            ...
            if (prop_meta) {
                ...
                if (target_index >= 0)
                    prop_meta.targets.splice(target_index, 1);
        ...
        function add_target(prop, target, junction_id) {
            ...
            if (prop_meta) {
                ...
                if (target.prop_id !== undefined)
                    obj.prop_id = target.prop_id;
    ...
    action_add_row(class_id, class_name) {
        ...
        if (class_name == undefined)
            class_name = this.class_cache[class_id].name;
    ...
    retrieve_class(class_id, class_name, class_meta) {
        ...
        let stringified_properties = class_meta.properties.filter(a => { var _a; return a.type == 'relation' || ((_a = a.conditions) === null || _a === void 0 ? void 0 : _a.max) > 1; });
        items.map(row => {
    ...
    retrieve_workspace_contents(id) {
        ...
        for (let block of blocks)
            block.properties = JSON.parse(block.properties);
    ...
    action_create_and_add_to_workspace(workspace_id, blocktype, block_properties, concept_data) {
        ...
        switch (blocktype) {
            case 'item':
                let { value: item_value, type: item_type } = concept_data;
