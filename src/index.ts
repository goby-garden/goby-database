...
export default class Project {
    ...
    create_junction_table(sides: JunctionSides) {
        ...
        if (defined(sides[0].prop_id)) columns.push(`"${side_0_col}_order" REAL`);
        if (defined(sides[1].prop_id)) columns.push(`"${side_1_col}_order" REAL`);
    ...
    action_edit_relations(
        ...
        const edited_data:Partial<Record<number,{
            modified_items:number[];
            modified_properties:number[];
            created_items:number[];
        }>>=[];

        function find_class_record(class_id:number){
            let edited_class=edited_data[class_id];
            if(!defined(edited_class)){
                edited_data[class_id]={
                    modified_items:[],
                    modified_properties:[],
                    created_items:[]
                };
                edited_class=edited_data[class_id];
            }

            return edited_class;
        }

        function record_modifications(input:RegisteredItemRelationSideInput,is_new:boolean = false){
            const edited_class=find_class_record(input.class_id);
            if(is_new){
            ...
            if (change == 'add') {
                ...
                const sides_registered:RegisteredItemRelationSideInput[]=sides.map((input)=>{
                    if("item_id" in input){
                        ...
                        record_modifications(newly_registered_item,true);
                        
                        return newly_registered_item;
                    }
                })
                ...
                sides_registered.forEach((side, s) => {
                    ...
                    if (defined(side.prop_id)) {
                        ...
                        if (defined(side.order)) {
                            ...
                            const item = this.retrieve_class_items({
                            ...
                            const prop_name = class_data.properties.find((p) => p.id == side.prop_id)?.name;
                            const prop_selections = defined(prop_name) && item ? item[`user_${prop_name}`] : [];

                            if (Array.isArray(prop_selections)) {
                                const last_item_order = prop_selections?.at(-1)?.system_order;
                ...
                if(("item_id" in input_1)&&("item_id" in input_2)){
                    ...
                    record_modifications(input_1);
                    record_modifications(input_2);
    ...
    retrieve_class_items({ class_id, class_name, class_data, pagination = {} }: { class_id: number, class_name?: string, class_data?: ClassData, pagination?: ItemPagination }): PaginatedItems {
        ...
        const cte_properties: (RelationProperty & { cte_only: boolean })[] = [];
        ...
        for (let condition of (pagination.conditions || [])) {
            if (condition.name == 'under_property_max') {
                ...
                if (property?.type == 'relation' && property.max_values !== null) {
                    ...
                    if (!cte_properties.some((p) => p.id == condition.property_id) && !retrieved_properties.some((p) => p.id == condition.property_id)) {
                        ...
                        cte_properties.push({ ...property, cte_only: true });
                    }
        ...
        for (let prop of [...relation_properties, ...cte_properties]) {
            ...
            if (prop.relation_targets.length > 0) {
                ...
                if (!("cte_only" in prop && prop.cte_only)) relation_selections.push(`[${prop.id}_cte].[user_${prop.name}]`);
                cte_joins.push(`LEFT JOIN [${prop.id}_cte] ON [${prop.id}_cte]."${property_junction_column_name}" = ${class_string}.system_id`)
            } else {
                if (!("cte_only" in prop && prop.cte_only)) relation_selections.push(`'[]' AS [user_${prop.name}]`);
            }
